...
module.exports = function (msg, args) {
  ...
  msg.channel.send(
    "PC Request command is disabled. Please use the webpage to reserve Zephyr equpment on https://www.akbot.app"
  );
  return;
  ...
  if (
    ...
    msg.channel.send(
      "As a board member you have to specify which slot you would like to cancel on the requested date with + " +
  ...
  if (args.length == 2 && args[0].toLowerCase() === "remove") {
    ...
    if (!(checkRole("Board", msg) || checkRole("Moderator", msg))) {
      msg.channel.send(
        "You do not have permissions for this command. You might want to use `cancel` instead of `remove`."
      );
      return;
    }
...
function reserveSlot(msg, EmbedBuilder) {
  ...
  if (!tokenUserData) {
    msg.channel.send(
      "There was an error with retrieving your data in the dataset." +
  ...
  if (
    ...
    msg.channel.send(
      "You can only reserve slots in the next month after the 15th."
    );
    return;
  }
  ...
  if (requestedEntryDate.slot1 !== "" && requestedEntryDate.slot2 !== "") {
    msg.channel.send(
      "The slots on the requested date are filled. If you are a board member than you can first cancel these reservations with `!pc cancel DD-MM slotnumber`"
    );
    return;
  }
  ...
  if (tokenUserData.tokens > 0) {
    ...
    msg.channel.send(
      "Slot " +
        ...
        " has been reserved, you can cancel this reservation with `!pc cancel DD-MM`"
    );
    printSchedule(msg, EmbedBuilder);
  } else {
...
function cancelSlot(msg, EmbedBuilder, args) {
  ...
  if (!checkRole("Board", msg)) {
    ...
    if (requestedEntryDate.slot1 === msg.author.id) {
      ...
      msg.channel.send(
        "You do not have any reserved slots on the requested date."
      );
      return;
    }
    ...
    msg.channel.send(
      "Slot " +
        ...
        " on " +
        dateObj.toLocaleDateString() +
        " has been canceled. You got your token back."
    );
  } else {
    let tokenSlotUser;
    if (args[2] === "1" && requestedEntryDate.slot1 !== "") {
      ...
      if (!tokenSlotUser) {
        msg.channel.send(
          "There was an error with retrieving the data of the user which reserved the slot, please contact AkBob if you encounter this issue."
        );
        return;
      }
    ...
    msg.channel.send(
      "The reservation on slot " +
        args[2] +
        " on " +
        dateObj.toLocaleDateString() +
        " was canceled and their token returned."
    );
  }
  printSchedule(msg, EmbedBuilder);
}
    ...
    .addFields(
      {
        name: "Mod commands",
        value:
          "!pc cancel DD-MM slotnumber\n\n\n\n!pc remove DD-MM\n\n\n\n!initSchedule\n",
        inline: true,
      },
      {
        name: "Description",
        value:
          "Cancel any reservation of the specified slot number on that date.\n\nRemove the date from the schedule and cancels any reservations on that date\n\nInitializes a new schedule for pc reservation for a new year",
        inline: true,
      }
    )
    .setTimestamp();
...
function checkDateFormat(date, msg) {
  ...
  if (!date.includes("-")) {
    msg.channel.send(
      "Wrong date format! Make sure to write it in this formate `DD-MM`. Example 31-01 for 31st of January."
    );
    return false;
  }
  ...
  if (
    !(
      parseInt(dateParts[0], 10) > 0 &&
      parseInt(dateParts[0], 10) <= 31 &&
      parseInt(dateParts[1], 10) > 0 &&
      parseInt(dateParts[1], 10) <= 12
    )
  ) {
    msg.channel.send(
      "The date requested was not a proper date format with correct numbers"
    );
    return false;
  }
  ...
  allDates.forEach((entryDate) => {
    ...
    if (
      trainingDay.getDate() == dateObj.getDate() &&
      trainingDay.getMonth() == dateObj.getMonth()
    ) {
      isTrainingDay = true;
...
async function printSchedule(msg, EmbedBuilder) {
  ...
  allDates.forEach((entry) => {
    ...
    if (
      ...
      dateObject.getMonth() <= now.getMonth() + 1 &&
      ...
      formattedSchedule.push(
        humanDate + ": Slot 1: " + slot1 + "; " + "Slot 2: " + slot2
      );
    }
  });
  ...
  if (formattedSchedule.length == 0) {
    msg.channel.send(
      "The schedule was empty. You might need to use `!initSchedule` if it is a new year."
    );
  } else {
    ...
    const scheduleEmbed = new EmbedBuilder()
      ...
      .setAuthor({ name: "AkBot", iconURL: "https://i.imgur.com/y21mVd6.png" })
      .setDescription(
        "If you want to reserve a slot on a specific date below, than use the command `!pc request DD-MM`. " +
      ...
      .setThumbnail("https://i.imgur.com/mXodbnH.png")
      .addFields({
        name: "Schedule next two months",
        value: "```" + formattedEmbedSchedule + "```",
      })
      .setTimestamp();
